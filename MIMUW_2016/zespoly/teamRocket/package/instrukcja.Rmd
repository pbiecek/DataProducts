...
roku 2015, doda do niego kilka dodatkowych wyznaczników i przekaże te
dane do aplikacji shiny.
...
2. Następnie wybierz wyznacznik - kryterium, ze względu na które chceszporównać wyniki maturalne uczniów wybranej placówki.

Możliwe jest filtrowanie gmin/szkół/wyznaczników poprzez wpisywanie części nazw w polach wyboru.

Przed Twoimi oczami ukaże się piękny histogram - wykres przedstawiający względną liczbę uczniów, dla których wyznacznik ma wartość przedstawioną na osi x.
...
skrot <- "suma_mat"
```
2. Formuły opisująca wartość wyznacznika.
m_mat_p i m_mat_r to nazwy kolumn w bazie danych
Listę wszystkich kolumn można uzyskać ze strony pomocy ?matura.2015
Kolumny używają takich samych nazw jak zdefiniowane w
...
```
3. Opisową nazwę wyznacznika
Będzie ona wyświetlona w aplikacji na liście wyboru wyznacznika,
oraz jako tytuł wykresu
```{r, eval=FALSE}
