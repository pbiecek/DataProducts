...
mini_classify <- mini_classify %>% rowwise() %>% mutate(wynik = 
                                                          max(0.5*p_mat*2, r_mat*2, na.rm = TRUE)*4/9 +
                                                          0.25 * max(0.5*p_ang*2,r_ang*2, na.rm = TRUE)/9 + 
                                                          max(r_fiz, 0.5*r_bio*10/6, r_fiz*10/6, 0.75*r_chem*10/6, r_inf*2, na.rm = TRUE)*4/9
)
uj_classify <- filter(data.wawa, p_mat>15 & p_pl>15 & sum(c(r_inf>0,r_mat>0, r_bio>0,r_fiz>0,r_chem>0), na.rm = TRUE)>1)
uj_classify <- mini_classify %>% rowwise() %>% mutate(wynik = 
                                                        max(max(0.5*p_mat*2, r_mat*2, na.rm = TRUE), 
                                                            max(0.5*p_inf*2, r_inf*2, na.rm = TRUE),
                                                            max(0.5*p_bio*10/6, r_bio*10/6, na.rm = TRUE),
                                                            max(0.5*p_fiz*10/6, r_fiz*10/6, na.rm = TRUE),
                                                            max(0.5*p_chem*10/6, r_chem*10/6, na.rm = TRUE),
                                                            na.rm = TRUE)/2 + 
                                                        
                                                        sort(
                                                          c(
                                                            max(0.5*p_mat*2, r_mat*2, na.rm = TRUE), 
                                                            max(0.5*p_inf*2, r_inf*2, na.rm = TRUE),
                                                            max(0.5*p_bio*10/6, r_bio*10/6, na.rm = TRUE),
                                                            max(0.5*p_fiz*10/6, r_fiz*10/6, na.rm = TRUE),
                                                            max(0.5*p_chem*10/6, r_chem*10/6, na.rm = TRUE)
...
uksford_classify <- uksford_classify %>% rowwise() %>% mutate(wynik = 
                                                                max(0.4*p_mat*2, 0.8*r_mat*2, na.rm = TRUE) +
                                                                max(0.1*p_ang*2, 0.2*r_ang*2, na.rm = TRUE)
)
...
shinyServer(function(input, output) {
  ...
  output$wykresSrzypcowy = renderPlot({
    ...
    top5 <- classify %>% group_by(id_szkoly) %>%
    ...
    top5$id_szkoly <- as.character(top5$id_szkoly)
    
    skrzypcowy <- ggplot(top5, aes(x = id_szkoly, y = wynik, group=id_szkoly, fill=nazwa_szkoly)) +
